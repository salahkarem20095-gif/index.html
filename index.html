...
<!-- شاشة البداية -->
<div id="startScreen">
    <h1>🔥 LEGENDARY MODE 🔥</h1>
    <button id="startBtn">ابدأ</button>
    <button id="settingsBtn">⚙️ إعدادات</button>
    <button id="colorBtnScreen">🛒 سلة الألوان</button>
</div>

<!-- شاشة الإعدادات -->
<div id="settingsScreen" class="hidden">
    <h2>⚙️ إعدادات</h2>
    <button id="mouseControlBtn">تحكم بالماوس</button>
    <button id="touchControlBtn">تحكم باللمس</button>
    <button id="settingsConfirmBtn" class="hidden">تأكيد التحكم</button>
    <button id="backBtn">رجوع</button>
</div>

<!-- شاشة اختيار الألوان -->
<div id="colorScreen" class="hidden">
    <h2>🛒 اختر ألوان الكرات</h2>
    <div id="colorOptions">
    ...
    <button id="colorConfirmBtn">تأكيد الألوان</button>
</div>

<!-- شاشة اختيار المستوى -->
<div id="levelScreen" class="hidden">
    <h2>اختر المستوى</h2>
    <button class="levelBtn" data-level="easy">سهل</button>
    <button class="levelBtn" data-level="normal">عادي</button>
    <button class="levelBtn" data-level="hard">صعب</button>
</div>
...
<button id="extraBtn">✨ Extra</button>

<div id="shop">
<h3>🛒 Shop</h3>
<button id="buySlow35">🐢 Slow Time (35 Coins)</button>
<button id="firePower">🔥 Fire Strike (50 Coins)</button>
</div>
...
document.getElementById("colorConfirmBtn").onclick = ()=>{
    if(selectedColors.length===0) alert("اختر على الأقل لون واحد!");
    else{
...
document.getElementById("pauseBtn").onclick=()=>{
    isPaused=!isPaused;
    document.getElementById("pauseBtn").textContent=isPaused?"▶ تشغيل":"⏸ إيقاف";
    if(!isPaused) update();
};
document.getElementById("extraBtn").onclick=()=>{
    balls.forEach(b=>{ if(!b.explosive) b.color=randomColor(); });
    alert("✨ Extra Ball Colors Activated!");
};
...
document.getElementById("firePower").onclick=()=>{
    if(fireUsed){ alert("🔥 Fire Strike already used!"); return; }
    if(coins>=50){
        ...
        alert("🔥 Fire Strike Activated for 5s!");
    } else alert("Not enough coins!");
};
